...
namespace Vitec.Controllers
{
    ...
    public class BUILDINGController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] BUILDING model)
        {
            var id = await _bUILDINGService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _bUILDINGService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _bUILDINGService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _bUILDINGService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] BUILDING updatedEntity)
        {
            ...
            var status = await _bUILDINGService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("BUILDING", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<BUILDING> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _bUILDINGService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}