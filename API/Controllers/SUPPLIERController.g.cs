...
namespace Vitec.Controllers
{
    ...
    public class SUPPLIERController : BaseApiController
    {
        ...
        {
            var id = await _sUPPLIERService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _sUPPLIERService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _sUPPLIERService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _sUPPLIERService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] SUPPLIER updatedEntity)
        {
            ...
            var status = await _sUPPLIERService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("SUPPLIER", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<SUPPLIER> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _sUPPLIERService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}