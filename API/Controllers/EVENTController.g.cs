...
namespace Vitec.Controllers
{
    ...
    public class EVENTController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] Entities.EVENT model)
        {
            var id = await _eVENTService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _eVENTService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _eVENTService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _eVENTService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] Entities.EVENT updatedEntity)
        {
            ...
            var status = await _eVENTService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("EVENT", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<Entities.EVENT> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _eVENTService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}