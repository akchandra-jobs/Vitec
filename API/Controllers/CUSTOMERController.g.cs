...
namespace Vitec.Controllers
{
    ...
    public class CUSTOMERController : BaseApiController
    {
        ...
        {
            var id = await _cUSTOMERService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _cUSTOMERService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _cUSTOMERService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _cUSTOMERService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] CUSTOMER updatedEntity)
        {
            ...
            var status = await _cUSTOMERService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("CUSTOMER", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<CUSTOMER> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _cUSTOMERService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}